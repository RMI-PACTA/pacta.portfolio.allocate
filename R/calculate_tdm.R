...
#' @param log_path String. A path leading to the desired logfile output.
#'
...
calculate_tdm <- function(data,
  ...
  check_calculate_tdm(data, t0, delta_t1, delta_t2, groups, log_path)
...
add_tdm <- function(data, delta_t1, delta_t2, groups) {
  data %>%
    ...
    select("tdm_technology_value_portfolio", "tdm_technology_value_scenario", all_of(groups)) %>%
    distinct() %>%
    pivot_longer(
...
check_calculate_tdm <- function(data, t0, delta_t1, delta_t2, groups, log_path) {
  check_crucial_names(data, crucial_columns())

  check_crucial_years(data, t0, delta_t1, delta_t2, groups)

  check_unique_by_year_and_groups(data, groups, log_path)

  invisible(data)
}

check_unique_by_year_and_groups <- function(data, groups, log_path) {
  ...
  if (!ok) {
    error_message <- "Data must be unique by year and groups for the following columns"

    write_log(error_message, file_path = log_path)
    rlang::abort(
      message = c(
        error_message,
...
check_crucial_years <- function(data, t0, delta_t1, delta_t2, groups) {
  ...
  missing_crucial_years <- crucial_years %>%
    left_join(data, by = c(crucial_years = "year"), multiple = "all") %>%
    group_by(!!!rlang::syms(groups)) %>%
  ...
  if (!ok) {
    error_message <- "Data must contain all crucial years, by group:"
